...
parser.add_argument('--cls_reg', type=float, default=0.1)
#python quality_plot_discarded.py --ds_path ../../../dataset/Synthetic --form ddnnf --dataset _0606 --atoms 6  --seed 2 --w_reg 0.1 --indep_weights
...
num_sol_selected = 64 # 64
num_fml_selected = 100 # 900
...
sol_data = f'sol_ddnnf{dataset}' if 'ddnnf' in form else f'sol_cnf{dataset}'
and_or = 'ddnnf' in form

print(f'Plot file will be saved as quality_plots/{filename}')
if not os.path.exists(f'./quality_plots/'):
    os.mkdir(f'./quality_plots/')
test_fmls = json.load(open(f'./testformula/{form}{dataset}{seed_name}.testformula'))
...
if args.use_old:
    ...
    adj0, features0, labels0, idx_train0, idx_val0, idx_test0, add_children0, or_children0 = load_data(
        f'{0}', form+dataset, and_or=and_or,override_path=ds_path)
    ...
    for sol_idx in tqdm(sol_list, desc="Embedding solutions"):
        adj1, features1, labels1, idx_train1, idx_val1, idx_test1, add_children1, or_children1 = load_data(
            f'{sol_idx}', sol_data, and_or=and_or,override_path=ds_path)
    ...
    for sol_idx in sol_list:
        file_sol = f'{ds_path}/sol_cnf{dataset}_raw/{sol_idx}.cnf'
    ...
    for formula_idx in tqdm(range(num_fmls), desc="Embedding formula"):
        adj0, features0, labels0, idx_train0, idx_val0, idx_test0, add_children0, or_children0 = load_data(
            ...
            form+dataset, and_or=and_or,override_path=ds_path)
    ...
    for formula_idx in range(num_fmls):
        ...
        file_cnf = f'{ds_path}/cnf{dataset}_raw/{test_fmls[formula_idx].split(".")[0]}.cnf'
...
#dist_flatten = normalize(dist_flatten)
