...
class MyDataset(Dataset):
    def __init__(self, dataset, dataset_split, and_or=True, args=None):
    ...
    def __getitem__(self, item):
        ...
        for item in range(5):
            try:
                adj0, features0, labels0, add_children0, or_children0 = self.load_data(
                    str(file_idx), self.dataset, and_or=self.and_or, directed = self.args.directed)
                adj1, features1, labels1, add_children1, or_children1 = self.load_data(
                    str(file_idx) + '.st' + str(item), self.dataset, and_or=self.and_or, directed = self.args.directed)
                adj2, features2, labels2, add_children2, or_children2 = self.load_data(
                ...
                data_item[-1].append(
                    (adj0.to_dense(), features0, labels0, add_children0, or_children0))

                data_item[-1].append(
                    (adj1.to_dense(), features1, labels1, add_children1, or_children1))

                data_item[-1].append(
                    (adj2.to_dense(), features2, labels2, add_children2, or_children2))
            except OSError as e:
                pass
    ...
    def load_data(self, filename, dataset, and_or=True, directed=True):
        ...
        labels = list(map(int,list(idx_features_labels[:, -1])))
        ...
        adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                            # shape=(self.adj_shape, self.adj_shape),
        ...
        return adj, features, labels, and_children, or_children
