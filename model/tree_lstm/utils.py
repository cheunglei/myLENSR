...
def load_data(filename=None, dataset=None, override_path=None, override_var=None, override_rel=None, and_or=True, directed=True):
    ...
    return tree, features, labels, idx_train, idx_val, idx_test, and_children, or_children
...
class Mydataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for item in range(5):
            try:
                tree0, features0, labels0, idx_train0, idx_val0, idx_test0, add_children0, or_children0 = self.load_data(
                    str(file_idx), self.dataset, and_or=self.and_or, directed = self.args.directed)
                tree1, features1, labels1, idx_train1, idx_val1, idx_test1, add_children1, or_children1 = self.load_data(
                    str(file_idx) + '.st' + str(item), self.dataset, and_or=self.and_or, directed = self.args.directed)
                tree2, features2, labels2, idx_train2, idx_val2, idx_test2, add_children2, or_children2 = self.load_data(
                ...
                data_item[-1].append(
                    (tree0, features0, labels0, idx_train0, idx_val0, idx_test0, add_children0, or_children0))

                data_item[-1].append(
                    (tree1, features1, labels1, idx_train1, idx_val1, idx_test1, add_children1, or_children1))

                data_item[-1].append(
                    (tree2, features2, labels2, idx_train2, idx_val2, idx_test2, add_children2, or_children2))
            except OSError as e:
                pass
...
def edges2tree(edges):
    ...
    for egde in edges:
        tree_list[egde[1]].add_child(tree_list[egde[0]])
