...
from models import TreeLSTM, MLP
...
def cuda_input(tree, features, labels, idx_train, idx_val, idx_test):
    if args.cuda:
    ...
    return tree, features, labels, idx_train, idx_val, idx_test
...
model = TreeLSTM(nfeat=features0.shape[1],
            nhid=args.hidden,
...
def train_step(epoch, loss_save):
    for _, data_train_group in tqdm(enumerate(dataset_train), desc='Training', total=len(dataset_train)):
        gc.collect()
        ...
        for data_train in data_train_group:
            ...
            for data_train_item in data_train:
                ...
                tree, features, labels, idx_train, idx_val, idx_test = cuda_input(*data_train_item[:-2])
                and_children, or_children = data_train_item[-2:]
                t = time.time()
                output = model(tree,features)
...
def test():
    ...
    model.eval()
    mlp.eval()
    for _, data_test_group in tqdm(enumerate(dataset_test), desc='Testing', total=len(dataset_test)):
        # pass three times first, then back propagate the loss
        for data_test in data_test_group:
            vector3 = []
            for data_test_item in data_test:
                tree, features, labels, idx_train, idx_val, idx_test = cuda_input(*data_test_item[:-2])
                output = model(tree,features)
                vector3.append(output[0])
...
for epoch in range(args.epochs):
    ...
    if test_stats[2] > best_acc:
        ...
        torch.save(model, './model_save/' + dataset + directed_name + cls_reg_name + seed_name + '.model')
        torch.save(mlp, './model_save/' + dataset  + directed_name + cls_reg_name + seed_name + '.mlp.model')
    ...
    #
    json.dump(train_loss_save, open('./acc_loss/' + dataset + directed_name + cls_reg_name + seed_name + '.train_save', 'w'),
              ensure_ascii=False)
    json.dump(test_loss_save, open('./acc_loss/' + dataset + directed_name + cls_reg_name + seed_name + '.test_save', 'w'),
              ensure_ascii=False)
    json.dump([int(np.argmax(test_loss_save['acc'])), max(test_loss_save['acc'])],
              open('./acc_loss/' + dataset + directed_name + cls_reg_name + seed_name + '.test_best', 'w'), ensure_ascii=False)

    pk.dump(loss_by_iter, open('./acc_loss/' + dataset + directed_name + cls_reg_name + seed_name + '.loss_by_iter', 'wb'))
