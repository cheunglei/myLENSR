...
class VRD_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(pairs)):
            key = pairs[i] # key = str(((label1, b1), (label2, b2)))
            ...
            if relation == 100:
                ...
                x.append(self.image_features_train[img][key]) # l1_l2_f = label1_vec + label2_vec + features
                y.append(relation)
                info.append(self.information[img][key]) # sub_iterate_info[key] = (img_name, (label1, b1), (label2, b2))
        x = Tensor(x)
        y = Tensor(y).long()
