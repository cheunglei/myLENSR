...
def iterate_images(iterate_info, iterate_annotation_train, iterate_train_img_dire, iterate_train_imgs,
                   iterate_preprocessed_annotation_train, iterate_preprocessed_image_features_train,train):
    for train_image in range(len(iterate_train_imgs)):
        ...
        sub_iterate_preprocessed_annotation_train = {}
        sub_iterate_preprocessed_image_features_train = {}
        sub_iterate_info = {}
        ...
        for i in range(len(img_obj)):
            for j in range(len(img_obj)):
                if (img_obj[i], img_obj[j]) not in relations_train_dic:
                    relations_train_dic[(img_obj[i], img_obj[j])] = 70 #“no-relation”.
        ...
        for item in relations_train_dic:
            ...
            #crop = transforms.functional.crop(img, left=xmin, top=ymin, height=ymax - ymin, width=xmax - xmin)
            ...
            sub_iterate_preprocessed_annotation_train[key] = relation
            sub_iterate_preprocessed_image_features_train[key] = l1_l2_f
            sub_iterate_info[key] = (img_name, (label1, b1), (label2, b2))
        if train :
            with open('../dataset/VRD/preprocessed_annotation_train.pk', 'ab') as f:
                pk.dump(img_name, f)
                pk.dump(sub_iterate_preprocessed_annotation_train, f)
            with open('../dataset/VRD/preprocessed_image_features_train.pk', 'ab') as f:
                pk.dump(img_name, f)
                pk.dump(sub_iterate_preprocessed_image_features_train, f)
            with open('../dataset/VRD/info_train.pk', 'ab') as f:
                pk.dump(img_name, f)
                pk.dump(sub_iterate_info, f)

        else :
            with open('../dataset/VRD/preprocessed_annotation_test.pk', 'ab') as f:
                pk.dump(img_name, f)
                pk.dump(sub_iterate_preprocessed_annotation_train, f)
            with open('../dataset/VRD/preprocessed_image_features_test.pk', 'ab') as f:
                pk.dump(img_name, f)
                pk.dump(sub_iterate_preprocessed_image_features_train, f)
            with open('../dataset/VRD/info_test.pk', 'ab') as f:
                pk.dump(img_name, f)
                pk.dump(sub_iterate_info, f)

    return iterate_preprocessed_annotation_train, iterate_preprocessed_image_features_train, iterate_info

##creat file and clean
...
#     pass
# with open('../dataset/VRD/info_train.pk', 'wb') as f:
#     pass
#
...
#                    preprocessed_image_features_train,train=True)
...
with open('../dataset/VRD/preprocessed_image_features_test.pk', 'wb') as f:
    pass
with open('../dataset/VRD/info_test.pk', 'wb') as f:
    pass
preprocessed_annotation_test, preprocessed_image_features_test, info_test = \
    iterate_images(info_test, annotation_test, test_img_dire, test_imgs, preprocessed_annotation_test,
                   preprocessed_image_features_test,train=False)
